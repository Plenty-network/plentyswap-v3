...
const test = () => {
  describe("factory.update_voter_address", () => {
    let tezos: Tezos;
    ...
    it("correctly updates the voter address", async () => {
      const factory = await tezos.deployContract("factory", storage);

      // When alice (the admin) changes the voter address
      await tezos.sendBatchOp([
        {
          kind: OpKind.TRANSACTION,
          ...factory.methods.update_voter_address(accounts.bob.pkh).toTransferParams(),
        },
      ]);
      ...
      expect(factoryStorage.voter).toEqual(accounts.bob.pkh);
    });

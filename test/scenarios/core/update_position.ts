...
const test = () => {
  describe("core.update_position", () => {
    ...
    it("correctly removes liquidity from a position", async () => {
      ...
      const collectedFeesX = Math2.bitShift(
        storage.fee_growth.x.multipliedBy(initialLiquidity),
        128
      );
      const collectedFeesY = Math2.bitShift(
        storage.fee_growth.y.multipliedBy(initialLiquidity),
        128
      );

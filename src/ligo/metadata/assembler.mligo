...
let token_metadata (token_id, store : nat * storage) : token_metadata_value =
    ...
    let (x_symbol, x_decimals) = 
        match Tezos.call_view "get_token_metadata" store.constants.token_x ("KT1J76gxNBWrYdRWnCmmA9o3MwSe14HsY7Qj": address) with
        | Some v -> v
        | None -> failwith invalid_contract in
    let (y_symbol, y_decimals) = 
        match Tezos.call_view "get_token_metadata" store.constants.token_y ("KT1J76gxNBWrYdRWnCmmA9o3MwSe14HsY7Qj": address) with
        | Some v -> v
        | None -> failwith invalid_contract in
    ...
    let background_seed = (Option.unopt (is_nat ((Tezos.get_now ()) - (0: timestamp)))) mod 3 in

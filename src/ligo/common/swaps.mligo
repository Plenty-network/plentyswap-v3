(* 
    ...
    https://github.com/tezos-checker/segmented-cfmm/blob/master/ligo/swap.mligo 
*)
...
let take_share (fee, (dev, protocol): nat * (nat * nat)) : nat * nat * nat = 
    ...
    let protocol_share = floordiv (fee * protocol) 10000n in
...
let rec x_to_y_rec (p : x_to_y_rec_param) : x_to_y_rec_param =
    if p.s.liquidity = 0n then
        ...
        if cur_tick_index_new.i >= p.s.cur_tick_witness.i then
            ...
            (* We want to overestimate how many tokens the user is putting into the pool (see Note [Rounding the swap result]), so we use `ceildiv`. *)
            ...
            (* Flip fee growth. This is fee growth 'above' the tick *)
...
let x_to_y (s : storage) (p : x_to_y_param) : result =
    ...
    if dy_received < p.min_dy then
        ...
        let op_receive_x = 
          cfmm_token_transfer (Tezos.get_sender ()) (Tezos.get_self_address ()) dx_spent s.constants.token_x in
        let op_send_y = 
          cfmm_token_transfer (Tezos.get_self_address ()) p.to_dy dy_received s.constants.token_y in
...
let y_to_x (s : storage) (p : y_to_x_param) : result =
    ...
    if dx_received < p.min_dx then
        ...
        let op_receive_y = 
          cfmm_token_transfer (Tezos.get_sender ()) (Tezos.get_self_address ()) dy_spent s.constants.token_y in
        let op_send_x = 
          cfmm_token_transfer (Tezos.get_self_address ()) p.to_dx dx_received s.constants.token_x in

(* Taken and modified from the original code at: 
  https://github.com/tezos-checker/segmented-cfmm/blob/master/ligo/types.mligo 
*)
...
type transfer_item = [@layout:comb] { 
    from_ : address;
...
type balance_request_item = [@layout:comb] { 
    owner : address;
    token_id : position_id;
}

type balance_response_item = [@layout:comb] { 
    request : balance_request_item;
    balance : nat
}

type balance_request_params = [@layout:comb] { 
...
type update_operator = [@layout:comb]
...
type fa2_parameter = [@layout:comb]
...
type tick_state = {
    ...
    *)
    ...
    (* sqrt(P) = sqrt(Y/X) associated with this tick. *)
    sqrt_price : x80n
}
...
type storage = {
    ...
    (* Square root of the virtual price, the value P for which P = Y/X. *)
    sqrt_price : x80n;
...
type update_position_param = {
    ...
    (* is -ve if liquidity is to be removed *)
    liquidity_delta : int;
